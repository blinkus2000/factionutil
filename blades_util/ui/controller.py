...
#Note: for the U.I. we will display Dict[Tuple[str, str], float] as a grid, such that Dict[Tuple[row, column], value]
#The UI will have a grid pinned to the upper left, top and to the top of the output console, each row label on the grid is a button that is linked to player_adjust_faction see below
#The output console will be pinned to the bottom left and right and to the bottom of the grid
#The input area will have the following buttons pinned to the far left stacked on top of each other
#Save,SaveAs,GenerateNew,AdvanceOneWeek
#in between the buttons and the grid will be a selectable list of available managers populated by get_available_managers
